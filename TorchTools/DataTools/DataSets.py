...
from .Prepro import _id, random_pre_process, random_pre_process_pair,random_pre_process_pair_lpkpn
...
class Lpkpndataset(data.Dataset):
    """
    Dataset for Real HL Pair
    Add Param For Image Enhance: No Change in Resolution
    :param rgb_range: 255. for RCAN and EDSR, 1. for others
    ...
    def __init__(self,
                 ...
                 mode='RGB',
                 scala=4,
                 prepro=random_pre_process_pair_lpkpn,
                 train=True,
                 need_hr_down=False,
                 need_lr_up=False,
                 need_edge_mask=False,
    ...
    def __getitem__(self, index):
        ...
        # print("**** self.lr_size ",self.lr_size)
        if self.train:
            hr_patch, lr_patch = self.prepro(hr_img, lr_img, self.lr_size, self.scale)
        else:
            hr_patch, lr_patch = self.prepro(hr_img, lr_img, self.lr_size, self.scale)
...
class RealPairDatasetTest(data.Dataset):
    """
    Dataset for Real HL Pair
    Add Param For Image Enhance: No Change in Resolution
    :param rgb_range: 255. for RCAN and EDSR, 1. for others
    ...
    def __init__(self,
        ...
        if need_lr:
            lr_file_path = os.path.join(lr_folder_path, 'train_LR') if train else lr_folder_path
            self.lr_file_list = _all_images(lr_file_path)
        if need_hr:
            hr_file_path = os.path.join(hr_folder_path, 'train_HR') if train else hr_folder_path
            self.hr_file_list = _all_images(hr_file_path)

        if need_lr:
            print('Initializing DataSet, image list: %s ...' % lr_folder_path)
            print('Found %d LR ...' % (len(self.lr_file_list)))
        if need_hr:
            print('Initializing DataSet, image list: %s ...' % hr_folder_path)
            print('Found %d HR ...' % (len(self.hr_file_list)))
        if need_sr:
            sr_file_path = os.path.join(hr_folder_path, 'train_SR') if train else os.path.join(hr_folder_path, 'test_SR')
            self.sr_file_list = _all_images(sr_file_path)
            print('Initializing DataSet, image list: %s ...' % hr_folder_path)
            print('Found %d SR...' % (len(self.sr_file_list)))
...
class SFMRealPairDataset(data.Dataset):
    """
    Dataset for Real HL Pair
    Add Param For Image Enhance: No Change in Resolution
    :param rgb_range: 255. for RCAN and EDSR, 1. for others
    ...
    def __init__(self,
                 ...
                 sfmflag=False,
                 sfmmode = 1,
                 SFM_center_radius_perc = -1,
                 SFM_center_sigma_perc = 0.05
                 ):
        lr_file_path = os.path.join(pair_folder_path, 'train_LR') if train else os.path.join(pair_folder_path, 'test_LR')
        hr_file_path = os.path.join(pair_folder_path, 'train_HR') if train else os.path.join(pair_folder_path, 'test_HR')
        self.lr_file_list = _all_images(lr_file_path)
        ...
        self.edge_mask_ch = edge_mask_ch
        self.need_sr = need_sr
        self.sfmflag=sfmflag
        self.sfmmode=sfmmode
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'Y':
            ...
            if self.need_sr:
                sr_patch = Func.to_tensor(sr_patch)[:1] * self.rgb_range
            if self.need_hr_down:
                hr_down_patch = Func.to_tensor(hr_patch_down)[:1]
            if self.need_lr_up:
                lr_patch_up = Func.to_tensor(lr_patch_up)[:1]
            if self.multiHR:
                hr_down_1 = Func.to_tensor(hr_down_1)[:1]
                hr_down_2 = Func.to_tensor(hr_down_2)[:1]

        else:
            lr_patch = Func.to_tensor(lr_patch) * self.rgb_range
            hr_patch = Func.to_tensor(hr_patch) * self.rgb_range
            if self.need_sr:
                sr_patch = Func.to_tensor(sr_patch) * self.rgb_range
            if self.need_hr_down:
                hr_patch_down = Func.to_tensor(hr_patch_down)
            if self.need_lr_up:
                lr_patch_up = Func.to_tensor(lr_patch_up)
            if self.multiHR:
                hr_down_1 = Func.to_tensor(hr_down_1)
                hr_down_2 = Func.to_tensor(hr_down_2)

        # print if or not sfm
        if (self.sfmflag):
            lr_patch, _ = random_drop(lr_patch, mode=self.sfmmode,
                                      SFM_center_radius_perc=self.SFM_center_radius_perc,
                                      SFM_center_sigma_perc=self.SFM_center_sigma_perc)
            print("*** sfm end...")
        ...
        if self.mode == 'Y':
            ...
            if self.need_sr:
                data['SR'] = sr_patch
            if self.need_hr_down:
                data['HR_DOWN'] = hr_patch_down
            if self.need_lr_up:
                data['LR_UP'] = lr_patch_up
            if self.multiHR:
                data['HR_D1'] = hr_down_1
                data['HR_D2'] = hr_down_2
