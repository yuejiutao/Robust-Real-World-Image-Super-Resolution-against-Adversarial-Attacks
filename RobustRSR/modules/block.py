...
class ShortcutBlockmc(nn.Module):
    #Elementwise sum the output of a submodule to its input
    def __init__(self, submodule):
        super(ShortcutBlockmc, self).__init__()
        self.sub = submodule
        self.premask=MaskBlockV2()

    def forward(self, input):
        x,Flag=input[0],input[1]
        if(Flag):
            x=self.premask(x)
        output = x + self.sub({0:x,1:Flag})[0]
        return {0:output,1:Flag}
...
class conv_blockmc(nn.Module):
    ...
    def __init__(self,in_nc, out_nc, kernel_size, stride=1, dilation=1, groups=1, bias=True, \
               pad_type='zero', norm_type=None, act_type='relu', mode='CNA'):
        super(conv_blockmc, self).__init__()
